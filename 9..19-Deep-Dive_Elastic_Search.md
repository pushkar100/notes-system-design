...
- [Elastic search](#elastic-search)
  ...
  - [Example system design diagram](#example-system-design-diagram)
  	- [Scatter/Gather pattern in a scaled ElasticSearch cluster](#scattergather-pattern-in-a-scaled-elasticsearch-cluster)
...
```
   ...
   /--------+------------------\
   |                           |
   | (1) Search Query          | (2) Create/Update Data
   |     ("iPhone 15")         |     (Transaction)
   |                           |
   v                           v
+==========+            +=========+
| Elastic- |            | Primary | <--- Source of Truth
| search   |            |   DB    |      (Postgres/MySQL)
| Cluster  |            |         |
+===+===+==+            +====+====+
    ^                       |
    ...
    |                       |
    |                 +-----v-----+
    +-----------------+  Indexer  | <--- (4) Async Write
                      |  Service  |      (Retries & Formatting)
                      +-----------+
```
A *horizontally scaled (clustered)* Elasticsearch cluster:
```
       +-------------+
       |  App Server |
       +------+------+
...
|             v                                                    |
|   +---------+-----------+                                        |
|   |  Coordinating Node  |  <--- Entry Point (Any node can be one)|
|   +----+----+-----+-----+                                        |
|        |    |     |                                              |
|        | (2) Scatter Phase                                       |
|        |    (Broadcast query to ALL relevant Shard IDs)          |
|        |    |     |                                              |
|        v    v     v                                              |
|   +----+-+  +----+-+  +------+                                   |
|   | Node |  | Node |  | Node | <--- Data Nodes (Hold Shards)     |
|   |  A   |  |  B   |  |  C   |                                   |
|   +------+  +------+  +------+                                   |
|   | [P0] |  | [P1] |  | [P2] | <--- Primary Shards               |
|   | [R1] |  | [R2] |  | [R0] |      (Write Scaling)              |
|   +------+  +------+  +------+                                   |
|      |         |         |                                       |
|      | (3) Local Search & Score                                  |
|      |     (Executed in parallel on Lucene instances)            |
|      |         |         |                                       |
|      +----+----+-----+---+                                       |
|           |    |     |                                           |
|           v    v     v                                           |
|   +-------+----+-----+--+                                        |
|   |  Coordinating Node  | <--- (4) Gather Phase                  |
|   +---------------------+      (Aggregate, Global Sort,          |
|                                 Fetch full docs, Return)         |
+==================================================================+
```

### Scatter/Gather pattern in a scaled ElasticSearch cluster

The Scatter/Gather Protocol (The Search Flow):
1. Say: "When a search query comes in, the receiving node acts as the Coordinating Node. It doesn't know which shard has the data, so it must **Scatter** the query to every shard ID in the index (parallel processing)."
2. Say: "Each data node executes the search locally using Lucene. They return just document IDs and scores. The coordinating node then **Gathers** these results, performs a global sort to find the 'top 10', and only then fetches the actual full documents to return to the user. This minimizes network traffic."
