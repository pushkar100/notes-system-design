...
**Note**: DynamoDB is primarily a Key-Value store, but it supports Document (JSON) data structures, making it a _hybrid_. It is Schema-less for everything except the Primary Key. You must define the Key upfront, but the rest of the data (Attributes) can be anything

The important distinction is not the product label but the question the database can answer efficiently. A key-value store answers, "Give me the value for this exact key." A document database can also inspect fields inside that value. A column-family store is organized around rows and access patterns at large scale, while a graph database makes relationships the primary thing to traverse.

```
  Same business object: a user profile

  Key-value:       "user:42" -----------------> [opaque bytes]
  Document:        "user:42" -----------------> {name, email, address}
  Column-family:   "user:42" -> [families and columns]
  Graph:           (User 42) --[LIVES_IN]--> (London)
```

The same data can therefore fit several models. The deciding factor is usually the dominant access pattern: how the application reads, writes, filters, and relates the data.
...
1. **_Persistence_**: They provided a standard way to save data so it survives after a power loss.
2. **_Concurrency_**: They handled many users reading/writing at the same time using Transactions.
3. **_Integration_**: They acted as a shared integration point for different applications
4. **_Standardization_**: SQL is a standard language. If you know SQL, you can work on almost any project
...
Here, **atomicity** means that the transfer is treated as one indivisible operation. **Durability** means a confirmed transfer survives a restart. **Isolation** controls what concurrent transfers are allowed to observe. **Consistency** means the operation preserves rules such as "an account balance cannot be negative"; that rule usually comes from the application's business logic and database constraints together.
...
- _Pro_: All data is in one place
- _Con_: It is very hard to change the database structure (schema) because changing it for the Shipping App might break the Accounting App
...
- Each application (or microservice) has its own private database. If the Accounting App needs shipping data, it asks the Shipping App **_via an API_** (web service); it never touches the Shipping DB directly
- _Pro_: **_You can choose the best database for the job_** (e.g., SQL for accounting, NoSQL for shipping logs)
- _Con_: You have to **_manage data syncing between apps yourself_**
...
- The Pain: **Translating a complex object (like a User with a list of Addresses and a set of Roles) into flat tables requires complex code or "Mapping" frameworks (like Hibernate)**. It feels _unnatural_ and _slows down development_
...
2. **Rigid Schema:** You _must define the structure (columns/types) upfront_. Changing it later (migrations) on a live database is slow and risky
3. **Impedance Mismatch**: They _don't map well to modern Object-Oriented code_ (requires **ORMs** like Hibernate) or unstructured data (JSON/XML).
4. **Performance at Scale**: _Joins become extremely slow as table sizes grow_ into the billions of rows
...
- _Scaling Up (Vertical)_: Buying a bigger, more expensive server with more RAM and CPU. **_RDBMS loves this_**
- _Scaling Out (Horizontal)_: Buying lots of cheap, small servers and connecting them ("Clustering"). **_NoSQL loves this_**
...
1. **_Joins are slow_**: Joining tables stored on different servers requires massive network traffic.
2. **_Transactions are hard_**: Keeping data consistent across 100 servers is very complex and slow.

> Keyword: **Sharding**. Splitting a database into smaller chunks (shards) and storing them across multiple servers

Sharding is not the same as replication. Sharding gives different nodes different portions of the dataset, while replication gives multiple nodes copies of the same portion. A production system commonly does both: first split users into shards, then keep each shard on several replicas.

```
  Partitioning:   users 1-1000  ---> Node A
                  users 1001-2000 -> Node B

  Replication:    Node A <------> Node A copy 1
                  Node A <------> Node A copy 2
```
...
2. **Cluster-Ready**: Designed from _day one_ to run on many servers (Scaling Out)
3. **Open Source**: Most major NoSQL DBs are open source
4. **Aggregate-Oriented**: They often store data the way the application sees it (e.g., _storing a User and their Addresses as one "Document" rather than splitting them into two tables_), **_solving the Impedance Mismatch_**
...
1. **The Relational Way (Shredding)**: In a relational database (SQL), data is "shredded" into small pieces. To save a "Customer," you might split their data across three tables. To read it back, you must `JOIN` them
   - _Con_: **_Joins are slow_**, especially across clusters.
   - _Pro_: Very flexible. You **_can query data from any angle_** i.e RDBMS is good for complex queries

2. **The Aggregate Way**: We _treat a collection of related objects as a single unit—an Aggregate_. We store the Customer, their Addresses, and their Preferences all in one "bucket"
   - _Pro_:
     - **_Fast to read/write the whole object (just one lookup)_**
     - **_Great for clusters (the whole aggregate lives on one server)_**
   - _Con_: **_Harder to query deep inside the data_** (e.g., "Find all users who have an address in Boston" is harder if addresses are buried inside a blob)
...
An aggregate boundary is both a data-modeling decision and a failure boundary. If an order and its line items belong to one aggregate, the database can often update them together. If the line items are spread across several aggregates, the application may need multiple writes, retries, and a way to repair a partially completed operation.

```
  One aggregate read:
  GET order:101 ---> { customer, status, items, total }

  Split across aggregates:
  GET order:101 ---> order record
  GET item:1    ---> item record       (more requests and coordination)
  GET item:2    ---> item record
```

This is why "store everything together" is not a universal rule. Put data together when it is bounded and usually used together. Keep it separate when it grows without limit, is shared by many objects, or needs independent update and query patterns.
...
2. **Rule of Atomicity (Writes)**: _NoSQL usually only guarantees ACID transactions within a single document_. If you need to update two things perfectly in sync (e.g., Order and OrderLineItems), they must be in the same aggregate
3. **Rule of Unbounded Growth (Size)**: _If a list can grow infinitely (e.g., a popular user's Followers or Comments), do not embed it_. Keep it separate to _prevent the document from hitting size limits_ (e.g., MongoDB's 16MB limit)
...
1. **Key-Value Model** (e.g., Riak, Redis)
   - The simplest NoSQL model. It acts like a **_giant Hash Map_** or **_Dictionary_**
   - The Key: The ID (e.g., `"session_123"`)
   - The Value: An _opaque blob_ (black box). The **_database does not know or care what is inside the value_**. It just stores the bytes
   - How you use it: _You can only get data by knowing the Key_. You cannot run a query like "Give me all blobs where city = NY"
...
2. **Document Model** (e.g., MongoDB)
   ...
   - The Key: The ID
   - The Document: The **_database understands the structure of the data inside_**
   - The Difference: Because the DB sees the structure, **_you can query internal fields_** (e.g., "Find documents where city is 'NY'")
...
**Note**: Key-Value and Document models are **_aggregate-oriented_** because they allow you to store, retrieve, and lock an entire complex object (like a User with all their Orders) as a single atomic unit, rather than forcing you to disassemble it into normalized rows across multiple tables

The word **opaque** is important here. In a key-value store, the database treats the value like a sealed package: it can route to the package using the key, but it cannot efficiently search for a field inside it. A document database opens the package and understands its fields, which enables field indexes and filters. That extra understanding provides flexibility, but it also creates more indexing and storage work.
...
Column-Family Stores (e.g., Cassandra, HBase): This is the **_most complex model_**, often called _"BigTable" style_. It looks like a table, but it acts like a _two-level nested map_
...
**Note**: Column-Family stores are **_aggregate-oriented_** because the Row Key acts as the root identifier for a massive, flexible structure (the Row), ensuring that all associated columns—even if they number in the millions—are physically stored together and retrieved as a single logical unit
...
Graph Data Models are **NOT** Aggregate-Oriented! They are rather **_relationship-oriented_** (similar to RDBMS) , as they deconstruct data into small individual nodes and edges to prioritize the complex connections between things rather than storing them as large, self-contained units
...
Concept: It doesn't mean there is _no_ schema; it means **_the database doesn't enforce one_**. The schema is implicit in your application code rather than explicit in the database engine. You can store data with different fields in the same bucket (bucket/table/collection), and it is up to your code to handle the differences. This **_allows you to change data structures without downtime_** (no `ALTER TABLE` commands)
...
In a NoSQL datastore (specifically `Key-Value` stores like Riak, Redis, or Couchbase), a **Bucket** is a **_logical container used to group data items together_**
...
_It acts as a **namespace** to keep keys organized_
...
Concept: A materialized view is **_a pre-calculated answer to a query, stored on disk as if it were a real table_**. Instead of running a slow, complex calculation every time someone asks a question, you calculate it once, save the result, and just read the saved result. **_It trades freshness for speed_** (the view might be slightly out of date).
...
This is an example of **denormalization**: storing the same fact in more than one shape so a common read is cheap. The cost is freshness and maintenance. If an order changes, the application must decide whether to update the materialized view immediately, update it through an event later, or rebuild it periodically.

```
  Write order ---> Orders store
       |
       +---------> Summary store ---> dashboard reads quickly

  One fact, two representations: fast reads require synchronization.
```

1. **In RDBMS (The "Magic" Way)**
   - In a relational database (like Oracle or PostgreSQL), a **_materialized view is a feature built into the database engine_**
   ...
   - Trade-off: **_It is easy for you, but it puts a heavy load on the database server (CPU/RAM)_**
2. **In NoSQL (The "Manual" Way)**
   - In the context of NoSQL Distilled, **_a materialized view is often a Design Pattern (something you build), not just a button you click_**. Because NoSQL databases are optimized for speed, they rarely do expensive "magic" background calculations for you
   - You do: **_You write code in your application_** (or use a stream processor). "When a user places an order, write the order to the Orders table AND update the TotalSales counter in the Analytics table."
   - The DB does: Just stores what you told it to store
   - Trade-off: **_It is harder to code_** (you have to ensure both writes happen), but it makes reading the data incredibly fast because the answer is already calculated

### Modeling for Data Access

Concept: **_In Relational databases, you model data based on what it is_** (**Entities: Users, Orders, Products**). **_In NoSQL, you model data based on how it is read_** (**Access Patterns**). You design your structures to perfectly match the specific questions your application asks, even if it means duplicating data.
...
- Goal: **_Scalability_**. It allows you to handle more data and traffic by adding more machines
- Trade-off: If one node goes down, that specific subset of data is unavailable

**B. Replication (Master-Slave vs. Peer-to-Peer)**

- **_Master-Slave_**: One node is the "Master" (handles writes); others are "Slaves" (read-only replicas)
  - Best for: **_Read-heavy workloads_**
  - Risk: **_Master is a single point of failure for writes_**
- **_Peer-to-Peer_** (**_Leaderless_**): Every node can handle reads and writes
  - Best for: **_High availability_** and **_write-heavy workloads_**
  - Risk: **_Dealing with write conflicts (concurrency)_**
...
**CAP** is about a distributed system under a network partition, meaning some nodes cannot communicate reliably with others. The three terms are **Consistency** (nodes return one agreed value), **Availability** (every request receives a response), and **Partition tolerance** (the system continues despite broken communication). Because partitions are possible in a real network, a system must choose which of consistency or availability it will prioritize during that failure.

```
  Node A  ----X----  Node B
          network partition

  Consistency choice: wait or reject until both sides agree
  Availability choice: answer locally, reconcile later
```

CAP is not saying that a database is simply "consistent" or "inconsistent" in every situation. It describes the behavior selected during a partition. Many systems offer stronger or weaker guarantees at different operation levels, so the application still needs to decide which reads and writes require the freshest value.

**A. Update Consistency**

- _Write-Write Conflict_: Two people update the same record at the exact same time.
...
This is the difference between **freshness** and **correctness**. A stale product-view count may be acceptable for a few seconds, but showing a user an old password-reset state is not. The required guarantee should come from the user-visible consequence, not from a blanket claim that one database is always better.

- _Logical Consistency_: Ensuring you see the data you just wrote (**_Read-Your-Own-Writes_**)
- _Replication Lag_: In a Master-Slave setup, a user might write to the Master but read from a Slave before the data has synced, seeing "stale" data
...
The symbols describe an overlap argument, not magic. `N` is the number of replicas holding a copy, `W` is the minimum acknowledgements for a successful write, and `R` is the minimum responses for a read. When `W + R > N`, the read set and write set must share at least one replica. That shared replica can tell the reader about the newer value.

- To ensure consistency in _Peer-to-Peer systems_, use `W + R > N`. `N` = Number of replicas. `W` = Number of nodes that must confirm a write. `R` = Number of nodes that must respond to a read
...
This guarantee depends on additional assumptions: replicas must compare versions correctly, a read must choose the newest version among its responses, and the system must handle concurrent writes. A quorum improves the chance of seeing fresh data; it does not by itself explain which value wins when two writers update the same key at the same time.

## Version Stamps

In distributed systems, we **_can't rely on a single central clock_**. We use Version Stamps to track changes and detect conflicts
...
- _Conflict Detection_: The system doesn't stop people from editing at the same time; it just detects when two people tried to change the same thing and makes the "late" person try again
...
An ordinary timestamp answers "when did this write happen according to one clock?" A vector clock answers a more useful distributed question: "which other writes had this node already seen?" It records a partial order of knowledge, so it can distinguish a later update from two independent updates that happened concurrently.

- The Problem: In Peer-to-Peer, two nodes might accept different writes for the same key simultaneously
- The Solution: A **_Vector Clock is a list of (node, counter) pairs_**.
...
A Key-Value (KV) store is the simplest form of NoSQL database. It acts like a **_giant Distributed Hash Map_** (or Dictionary).
...
- Access: **_You can only query by the Key_**. You cannot ask the database to "find all users where age > 25" because the database cannot see inside the "blob"
...
The key is not just a label; it is the routing decision. In a distributed key-value store, a hash or range function maps the key to the node responsible for it. A well-designed key spreads traffic, while a predictable or extremely popular key can create a **hotspot** where one node does most of the work.

```
  hash("user:42") -> partition 3 -> Node C
  hash("user:43") -> partition 1 -> Node A

  Request path: key -> partition -> node -> value
```

1. _Consistency_: Usually **_only consistent for operations on a single key_**.
2. _Transactions_: Most KV stores **_do not support multi-key transactions_**. If you update Key A and Key B, there is no guarantee both happen together.
3. _Querying_: Limited to `GET`, `PUT`, and `DELETE` **_by Key_**. To search by value, you would have to pull all data into your application and filter it yourself (very slow).
4. _Scalability_: Extremely easy to scale via Sharding. Since keys are independent, you just hash the key and send it to a specific server
...
Caching introduces another copy of the data, so the central question becomes **invalidation**: when the source changes, how does the cached value become old or get removed? A cache hit is fast because it avoids the primary store; a cache miss or stale entry is the price of that shortcut.
...
```
  Read request ---> Cache hit? --yes--> return cached value
                         ...
                         no
                         v
                    Primary store ---> save in cache ---> return value
```
...
1. **Relationships between Data**: If you need to link "Users" to "Orders" to "Products," a KV store is painful because it **_doesn't support Joins_**
...
3. **Querying by Data**: If your UI requires **_complex filters_** (e.g., "Find all blue shirts under $20")
...
A Document Database **_stores data as documents (usually JSON, BSON, or XML)_**. Unlike Key-Value stores, **_the "Value" is not opaque_**; it is **_a structured object that the database can inspect, index, and query_**

- Self-Describing: Each document contains **_both the data and the labels_** for that data
- _Hierarchical_: Documents can contain **_nested_** lists and other documents (Aggregates)
...
The practical difference from key-value storage is **content awareness**. The database can parse the document, find nested fields, and maintain indexes for them. This makes queries more expressive, but every index must be maintained when a document changes. Embedding also means that updating a shared fact may require updating many documents.

1. **Content Awareness**: Because the DB understands the structure, you **_can query specific fields_** (e.g., `db.orders.find({"customer": "Alice"})`).
2. **Indexing**: You can create indexes on nested fields (like the "price" inside the "items" array) to make searches faster
3. **Schema Flexibility**: **_Every document in a collection can have different fields_**. You can add a `"discount_code"` field to new orders without affecting old ones
4. **Aggregate-Oriented**: **_Related data is stored together_**. Instead of joining a Users table and an Addresses table, you store the address inside the user document
...
1. **Content Management Systems (CMS)**: Different types of content (articles, videos, images) have _different metadata but need to live in the same "Posts" collection_
...
1. **Complex Multi-Document Joins**: If your data is **_highly normalized and you constantly need to link many different documents together_**, the "Join-on-the-fly" performance is poor compared to RDBMS
2. **Strict Consistency across multiple documents**: While most Document DBs are ACID-compliant at the single document level, they **_may struggle with transactions involving many different collections_**
...
Column-family terminology can be confusing because it resembles a relational table. The important difference is that the row key is the main access path, and the database is designed around known queries for that row key. It is usually not a general-purpose replacement for SQL joins or ad-hoc filtering across every column.

1. **Sparse Storage**: If a row doesn't have data for a specific column, it takes up **_zero space_**. **_You don't store "`NULL`s" like in SQL_**
2. **High Write Throughput**: They are optimized for writes (using LSM-Trees), making them **_much faster than RDBMS for heavy ingestion_**
3. **Physical Separation**: **_Different Column Families are stored in separate files_**. If you only need "Name," the database doesn't have to touch the "Activity" files
4. **Versioning**: **_Every cell has a timestamp_**. You can store multiple versions of the same column (e.g., the last 3 addresses of a user)
...
**TTL (Time To Live)** attaches an expiration duration to a value. It is useful for sessions, temporary tokens, and telemetry that has a limited business lifetime. TTL cleanup may happen asynchronously, so applications should still treat an expired item as unavailable when reading it rather than depending on deletion to happen at an exact instant. 4. **Big Data Analytics**: When you have billions of rows but only need to read a few specific columns (the "Columnar" nature makes this fast)
...
1. **Early Stage Prototyping**: These databases require you to **_know your query patterns upfront_**. If you don't know how you'll need to read the data, you’ll struggle
2. **ACID Transactions**: They typically **_do not support multi-row transactions_**
3. **Complex Joins**: Like most NoSQL, **_they don't do joins_**. You must denormalize your data (duplicate it) into separate tables to satisfy different queries
...
Graph databases are presented as the **_"exception" to the aggregate-oriented rule_**. While other NoSQL types group data into "blobs," **_Graphs focus on the connections between data points_**
...
Graph performance comes from following an existing edge rather than repeatedly discovering a relationship with joins. A **traversal** is a walk through one or more edges: Alice -> follows -> Bob -> bought -> Book. The number of hops and the number of paths explored often matter more than the total number of nodes in the graph.
...
Think of a schema migration as a compatibility problem, not only a database command. During a deployment, old application instances and new application instances may run at the same time. A safe transition therefore often has three stages: make the new field readable, start writing the new field, and only later remove the old representation.

```
  Deploy 1: old and new code can read
  Deploy 2: new code writes old + new fields
  Backfill: old records receive the new field
  Deploy 3: new code relies on the new field
  Cleanup: remove the old field when no reader needs it
```

Every successful application eventually needs to change its data. Whether you are adding a new feature (e.g., "Add Social Media links to User Profiles") or fixing a design flaw, you have to **_transition_** from **Version A** to **Version B**

- The Problem: You have millions of existing records in the "Version A" format
- The Goal: **_Ensure the application doesn't break while transitioning_** to "Version B"

### Schema Changes in RDBMS

In a Relational Database, the **_schema is explicit_**. The **_database engine "owns" the structure_**

- Process: **_You use Migrations (scripts)_**. You run an `ALTER TABLE` command to add columns or change types
- The Downside:
  1.  _Downtime_: On large tables, `ALTER TABLE` can lock the database for hours
  2.  _All-or-Nothing_: Every single row must conform to the new schema immediately. There is no "middle ground"
...
In NoSQL, the **_schema is implicit_**. The **_application code "owns" the structure_**. This allows for **_Incremental Migrations_**, where different versions of data _coexist_

**A. Incremental Migration (Lazy Migration)**

- Instead of updating all 1 million rows at once, you update them **_only when they are accessed_**
...
Because NoSQL allows different versions to live together, you can run into "**_Gremlins_**"—old code trying to read new data, or new code encountering ancient data it doesn't know how to handle. **_You must use Feature Toggles or Conditional Logic in your code_**
...
This boundary is an example of **encapsulation**. The service owns the database schema and exposes a smaller business-level interface, such as `getRecommendations(userId)`. Callers depend on that behavior rather than on collection names, indexes, or query syntax. That reduces coupling and makes a storage change local instead of a whole-application rewrite.
...
Polyglot persistence **_isn't free_**. It adds Organizational Debt:
...
1. **Encapsulation**: Always wrap your data stores in **_services_** to hide the polyglot complexity from the rest of the business logic
2. **Don't Over-Engineer**: Just because you can use 5 databases doesn't mean you should. **_Start with one and add others only when the performance or functional gain is significant_**
3. **Reporting**: Polyglot persistence makes reporting hard because data is scattered. You often need a **_Data Lake or ETL process to pull everything into one place for the business analysts_**
