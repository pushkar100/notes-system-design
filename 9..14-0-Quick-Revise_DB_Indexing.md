...
**Note**: If you create a **Composite Index** on `(A, B, C)`, the database stores the data sorted first by A, then by B, then by C
- Because of the **Leftmost Prefix Rule** you can only use the index if you start searching from the beginning (Left) without skipping columns
- Valid Combinations (Index works)
  - `A` only: WHERE A = 1 (Works perfectly. It just reads the first level of the sort).
  - `A + B`	: WHERE A = 1 AND B = 2 (Works perfectly. Finds A=1, then jumps to B=2).
  - `A + B + C`: WHERE A = 1 AND B = 2 AND C = 3 (Works perfectly. Uses the full index)
- Invalid / Partial Combinations
  - `B` only: WHERE B = 2 -> Index NOT used (The DB is sorted by A. Asking for B is like looking for the 2nd letter of a word in a dictionary without knowing the first letter. You have to scan everything)
  - `C` only: WHERE C = 3 -> Index NOT used
  - `B + C`: WHERE B = 2 AND C = 3 -> Index NOT used
  - `A + C`: WHERE A = 1 AND C = 3 -> **Partial Use** (The DB uses the index to find A=1. But since you skipped B, it cannot jump to C. It must scan all rows where A=1 to find C=3)
