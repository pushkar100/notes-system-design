...
```
    ...
    +-------+--------+                     |
            |                              | (1) INCR "user_id_hits"
            | (2) Forward only             |     GET "user_session"
            |     if limit not             |
            |     exceeded                 v
            v                       +------+------+
    +-------+--------+              |    Redis    |
...
A *horizontally scaled* Redis server example:
```
       +------------------+
       | Redis App Server |
       | (Cluster         | <--- (1) Smart Client / Proxy
       |  Aware Lib)      |      (Calculates CRC16(key) % 16384)
       +------+-----------+
              |
...
+-------+ +-------+ +-------+
    |         |         |
+=======+ +=======+ +=======+
| MSTR  | | MSTR  | | MSTR  | <--- (2) Write Operations
| Node A| | Node B| | Node C|      (Keys A-H)
+---+---+ +---+---+ +---+---+
    | ^       | ^       | ^
    | |       | |       | |    (3) Async Replication
    v |       v |       v |    & Failover Pings (Gossip)
+---+---+ +---+---+ +---+---+
| REPL  | | REPL  | | REPL  | <--- (4) Read Scalability /
| Node A| | Node B| | Node C|      High Availability
+-------+ +-------+ +-------+
```
